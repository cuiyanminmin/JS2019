...
let flag = false;//代表新数据渲染完成;什么时候flag应该是true;新数据一请求，就把flag置为
let n =0;//记录加载新数据的次数；
function getData(){
    flag = true;
    n++;
    var xhr = new XMLHttpRequest();
    xhr.open('get','./data.json',true);//true异步
...
function render(ary){
    ...
    ary.forEach((item,index)=>{
       ...
       /*  str = ` <div class="img_box">
        <img realSrc="${pic}" src="./img/6.jpg" alt="" style='height:${height}px'>
        <p class="desc">${desc}</p>
