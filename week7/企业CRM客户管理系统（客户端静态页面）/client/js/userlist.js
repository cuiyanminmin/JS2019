$(function(){
    let $tbody = $('.tableBox tbody'),
        ...
        $handle = $('.tabelBox thead .w12'),
        $selectItems = null,
        $deleteBtns = null,
        $resetBtns = null;

    let canshow = true;//用来判断当前这个人有没有操作权限
    function role(){//角色
        //按照resetpassword 这个权限进行判断
    ...
    function eventBind(){
        ...
        $resetBtns.on('click',function(id){
            alert()
        })
    }
    ...
    $('.searchInp').on('keydown',function(e){
        if(e.keyCode ==13){
        //敲的回车键
        getData({departmentId:$('.selectBox')[0].value,
        search:this.value
    })
    this.value ='';//敲回车清空内容
        }
    ...
    function batchDelete(){
        ...
        //items中存放的 就是选中的那几个 tr
        //filter返回true就把当前项放在items（新数组中），返回false就不放了
        let ary =[];
        items.forEach(item=>{
            //获取要删除的这条数据的ID
